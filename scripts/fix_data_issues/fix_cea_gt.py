...
if __name__ == '__main__':
    cea_gt = join(realpath('.'), 'gt', 'CEA_biodivtab_2021_gt.csv')
    df = pd.read_csv(cea_gt, header=None, index_col=None, names=['filename', 'col', 'row', 'solution'])
    print(df.head(10))
    ...
    df.to_csv('new_cea_gt.csv', index=None, header=None)